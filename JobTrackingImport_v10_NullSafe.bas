...
Private Sub ApplyShippingUpdates(ByVal db As DAO.Database, ByVal runID As Long)
    ...
    Dim rsSource As DAO.Recordset, rsJob As DAO.Recordset, rsShipment As DAO.Recordset, rsExisting As DAO.Recordset
    ...
    ' Access refuses every kind of join against that datatype.
    Set rsSource = db.OpenRecordset("SELECT * FROM [" & STAGE_SHIPPING & "]", dbOpenSnapshot)
    Set rsShipment = db.OpenRecordset("ShipmentTracking", dbOpenDynaset)
    Do While Not rsSource.EOF
        ...
        If Not IsNull(shippedValue) Then
            If IsDate(shippedValue) Then
                Set rsJob = FindShippingJob(db, consultValue, poValue)
                If Not rsJob Is Nothing Then
                    ...
                    Set rsExisting = db.OpenRecordset("SELECT ShipmentID FROM ShipmentTracking WHERE JobOrderID=" & CStr(rsJob!JobOrderID) & " AND ShippedDate=#" & Format(CDate(shippedValue), "yyyy/mm/dd hh:nn:ss") & "#", dbOpenSnapshot)
                    If rsExisting.EOF Then
                        rsShipment.AddNew
                        rsShipment!JobOrderID = rsJob!JobOrderID
                        rsShipment!ShippedDate = CDate(shippedValue)
                        If Not IsNull(trackingValue) Then rsShipment!TrackingNumber = CStr(trackingValue)
                        If Not IsNull(invoiceValue) Then rsShipment!InvoiceNumber = CStr(invoiceValue)
                        If Not IsNull(frameValue) Then rsShipment!FrameName = CStr(frameValue)
                        If Not IsNull(lensValue) Then rsShipment!LensDescription = CStr(lensValue)
                        rsShipment.Update
                    End If
                    rsExisting.Close
                    rsJob.Edit
                    rsJob!ClosedDate = CDate(shippedValue)
                    rsJob!JobStatus = "Closed"
                    If Not IsNull(frameValue) Then rsJob!FrameName = CStr(frameValue)
                    If Not IsNull(lensValue) Then rsJob!LensDescription = CStr(lensValue)
                    rsJob.Update
                    End If
                    rsJob.Close
                End If
