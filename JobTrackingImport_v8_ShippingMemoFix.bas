...
Private Sub ApplyShippingUpdates(ByVal db As DAO.Database, ByVal runID As Long)
    ...
    ' Access cannot join using OR conditions or Long Text/Memo source fields.
    ' Match consult-number rows first, then use PO only when no consult number is supplied.
    If cn <> "Null" Then
        InsertShipmentsByMatch db, "J.ConsultNumber=CLng(Val(Nz(" & cn & ",0)))", cn & " Is Not Null", shipped, tracking, invoice, frame, lens
        CloseJobsByMatch db, "J.ConsultNumber=CLng(Val(Nz(" & cn & ",0)))", cn & " Is Not Null", shipped, frame, lens
    End If
    If po <> "Null" Then
        Dim poOnly As String, safePO As String
        poOnly = ""
        If cn <> "Null" Then poOnly = " AND " & cn & " Is Null"
        safePO = MakeShortTextCopy(db, STAGE_SHIPPING, po, "ImportPONumber")
        InsertShipmentsByMatch db, "J.PONumber=" & safePO, safePO & " <> ''" & poOnly, shipped, tracking, invoice, frame, lens
        CloseJobsByMatch db, "J.PONumber=" & safePO, safePO & " <> ''" & poOnly, shipped, frame, lens
    End If
End Sub

' Excel occasionally imports PO as Long Text/Memo. Access cannot join to that
' datatype, so copy it into a local Short Text field before matching JobOrder.
Private Function MakeShortTextCopy(ByVal db As DAO.Database, ByVal tableName As String, ByVal sourceRef As String, ByVal newFieldName As String) As String
    Dim bareSource As String
    bareSource = Replace(sourceRef, "S.", "")
    If Not HasField(db, tableName, newFieldName) Then
        db.Execute "ALTER TABLE [" & tableName & "] ADD COLUMN [" & newFieldName & "] TEXT(255)", dbFailOnError
    End If
    db.Execute "UPDATE [" & tableName & "] SET [" & newFieldName & "]=Left(CStr(Nz(" & bareSource & ",'')),255)", dbFailOnError
    MakeShortTextCopy = "S.[" & newFieldName & "]"
End Function
