...
Public Sub ImportDailyJobTrackingWorkbook()
    ...
    ImportSheet sourceFile, "InProgress", STAGE_INPROCESS
...
Private Sub ValidateImport(ByVal db As DAO.Database, ByVal runID As Long)
    ...
    If RowCount(db, STAGE_INPROCESS) > 0 Then
        ...
        If consultField = "Null" Or poField = "Null" Then AddIssue db, runID, "Block", "InProgress", "", "InProgress sheet needs both Consult Number and PO Number."
    End If
...
Private Function FieldRef(ByVal db As DAO.Database, ByVal tableName As String, ByVal aliasName As String, ParamArray choices()) As String
    ...
    For Each choice In choices
        If HasField(db, tableName, CStr(choice)) Then FieldRef = aliasName & ".[" & Replace(CStr(choice), "]", "]]" ) & "]": Exit Function
    Next choice
    FieldRef = "Null"
End Function
