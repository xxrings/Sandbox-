...
Private Sub ApplyShippingUpdates(ByVal db As DAO.Database, ByVal runID As Long)
    ...
    End If
    If po <> "Null" Then
        Dim poOnly As String
        poOnly = ""
        If cn <> "Null" Then poOnly = " AND " & cn & " Is Null"
        InsertShipmentsByMatch db, "J.PONumber=CStr(Nz(" & po & ",''))", po & " Is Not Null" & poOnly, shipped, tracking, invoice, frame, lens
        CloseJobsByMatch db, "J.PONumber=CStr(Nz(" & po & ",''))", po & " Is Not Null" & poOnly, shipped, frame, lens
    End If
End Sub
