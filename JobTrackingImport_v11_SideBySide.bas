Attribute VB_Name = "JobTrackingImport_V11"
...
Public Sub InstallJobTrackingImporter_V11()
    ...
    On Error GoTo 0
    MsgBox "Installer complete. Run ImportDailyJobTrackingWorkbook_V11 to select and validate a workbook.", vbInformation
End Sub

Public Sub ImportDailyJobTrackingWorkbook_V11()
    ...
    ' TransferSpreadsheet creates new local tables. CurrentDb returns a fresh
    ' DAO object so the staging TableDefs collection includes those new tables.
    Set db = CurrentDb
...
Public Sub CommitDailyJobTrackingImport_V11()
...
Private Sub UpsertConsults(ByVal db As DAO.Database)
    ...
    Dim setSql As String, insertFields As String, insertValues As String
    ...
    AddSetField db, "Consult", setSql, "PatientDisplay", patient
    AddSetField db, "Consult", setSql, "Last5", last5
    AddSetField db, "Consult", setSql, "CreatedDate", created
    AddSetField db, "Consult", setSql, "ServiceName", service
    AddSetField db, "Consult", setSql, "ConsultStatus", status
    AddSetField db, "Consult", setSql, "County", county
    If Len(setSql) > 0 Then db.Execute "UPDATE [Consult] AS C INNER JOIN [" & STAGE_CTM & "] AS S ON C.[ConsultNumber]=" & cn & " SET " & setSql, dbFailOnError

    AddInsertField db, "Consult", insertFields, insertValues, "ConsultNumber", cn
    AddInsertField db, "Consult", insertFields, insertValues, "PatientDisplay", patient
    AddInsertField db, "Consult", insertFields, insertValues, "Last5", last5
    AddInsertField db, "Consult", insertFields, insertValues, "CreatedDate", created
    AddInsertField db, "Consult", insertFields, insertValues, "ServiceName", service
    AddInsertField db, "Consult", insertFields, insertValues, "ConsultStatus", status
    AddInsertField db, "Consult", insertFields, insertValues, "County", county
    db.Execute "INSERT INTO [Consult] (" & insertFields & ") SELECT " & insertValues & " FROM [" & STAGE_CTM & "] AS S LEFT JOIN [Consult] AS C ON C.[ConsultNumber]=" & cn & " WHERE C.[ConsultNumber] Is Null", dbFailOnError
End Sub

Private Sub AddSetField(ByVal db As DAO.Database, ByVal tableName As String, ByRef setSql As String, ByVal fieldName As String, ByVal sourceExpression As String)
    If Not HasField(db, tableName, fieldName) Then Exit Sub
    If Len(setSql) > 0 Then setSql = setSql & ", "
    setSql = setSql & "C.[" & fieldName & "]=" & sourceExpression
End Sub

Private Sub AddInsertField(ByVal db As DAO.Database, ByVal tableName As String, ByRef insertFields As String, ByRef insertValues As String, ByVal fieldName As String, ByVal sourceExpression As String)
    If Not HasField(db, tableName, fieldName) Then Exit Sub
    If Len(insertFields) > 0 Then insertFields = insertFields & ",": insertValues = insertValues & ","
    insertFields = insertFields & "[" & fieldName & "]"
    insertValues = insertValues & sourceExpression
End Sub
...
Private Sub ApplyInProcessUpdates(ByVal db As DAO.Database)
    Dim cn As String, po As String, consultMatch As String
    ...
    ' The Excel export may import Consult Number as text; Val/Nz makes the
    ' comparison safe for blank cells and text-form numeric values.
    consultMatch = "J.[ConsultNumber]=CLng(Val(Nz(" & cn & ",'')))"
    db.Execute "UPDATE JobOrder AS J INNER JOIN [" & STAGE_INPROCESS & "] AS S ON " & consultMatch & " SET J.[PONumber]=" & po & " WHERE IsNumeric(" & cn & ") AND " & po & " Is Not Null", dbFailOnError
End Sub
...
Private Sub ApplyShippingUpdates(ByVal db As DAO.Database, ByVal runID As Long)
    ...
    Dim consultValue As Variant, poValue As Variant, shippedValue As Variant, trackingValue As Variant, invoiceValue As Variant, frameValue As Variant, lensValue As Variant
    Dim jobByConsult As Object, jobByPO As Object, shipmentKeys As Object, jobID As Long, shipmentKey As String, sql As String
    ...
    ' Do not use a SQL join here. Excel can import PO as Long Text/Memo and
    ' Access refuses every kind of join against that datatype. Load lookup keys
    ' once, then use short direct updates; this avoids long network waits.
    Set jobByConsult = CreateObject("Scripting.Dictionary")
    Set jobByPO = CreateObject("Scripting.Dictionary")
    Set shipmentKeys = CreateObject("Scripting.Dictionary")
    LoadJobLookup db, jobByConsult, jobByPO
    LoadShipmentKeys db, shipmentKeys
    Set rsSource = db.OpenRecordset("SELECT * FROM [" & STAGE_SHIPPING & "]", dbOpenSnapshot)
    Do While Not rsSource.EOF
        consultValue = ValueFromStage(rsSource, cn)
        poValue = ValueFromStage(rsSource, po)
        shippedValue = ValueFromStage(rsSource, shipped)
        If Not IsNull(shippedValue) Then
            If IsDate(shippedValue) Then
                jobID = ShippingJobID(jobByConsult, jobByPO, consultValue, poValue)
                If jobID <> 0 Then
                    trackingValue = ValueFromStage(rsSource, tracking): invoiceValue = ValueFromStage(rsSource, invoice)
                    frameValue = ValueFromStage(rsSource, frame): lensValue = ValueFromStage(rsSource, lens)
                    shipmentKey = CStr(jobID) & "|" & Format(CDate(shippedValue), "yyyymmddhhnnss")
                    If Not shipmentKeys.Exists(shipmentKey) Then
                        sql = "INSERT INTO ShipmentTracking (JobOrderID,ShippedDate,TrackingNumber,InvoiceNumber,FrameName,LensDescription) VALUES (" & CStr(jobID) & "," & SqlDate(CDate(shippedValue)) & "," & SqlValue(trackingValue) & "," & SqlValue(invoiceValue) & "," & SqlValue(frameValue) & "," & SqlValue(lensValue) & ")"
                        db.Execute sql, dbFailOnError
                        shipmentKeys.Add shipmentKey, True
                    End If
                    sql = "UPDATE JobOrder SET ClosedDate=" & SqlDate(CDate(shippedValue)) & ", JobStatus='Closed'"
                    If Not IsNull(frameValue) Then sql = sql & ", FrameName=" & SqlValue(frameValue)
                    If Not IsNull(lensValue) Then sql = sql & ", LensDescription=" & SqlValue(lensValue)
                    db.Execute sql & " WHERE JobOrderID=" & CStr(jobID), dbFailOnError
                End If
            End If
        End If
        rsSource.MoveNext
...
Private Sub LoadJobLookup(ByVal db As DAO.Database, ByVal jobByConsult As Object, ByVal jobByPO As Object)
    Dim rs As DAO.Recordset, key As String
    Set rs = db.OpenRecordset("SELECT JobOrderID, ConsultNumber, PONumber FROM JobOrder", dbOpenSnapshot)
    Do While Not rs.EOF
        If Not IsNull(rs!ConsultNumber) Then jobByConsult("C:" & CStr(rs!ConsultNumber)) = CLng(rs!JobOrderID)
        If Not IsNull(rs!PONumber) Then
            key = "P:" & UCase$(Trim$(CStr(rs!PONumber)))
            If Len(key) > 2 And Not jobByPO.Exists(key) Then jobByPO.Add key, CLng(rs!JobOrderID)
        End If
        rs.MoveNext
    Loop
    rs.Close
...
Private Sub LoadShipmentKeys(ByVal db As DAO.Database, ByVal shipmentKeys As Object)
    Dim rs As DAO.Recordset
    Set rs = db.OpenRecordset("SELECT JobOrderID, ShippedDate FROM ShipmentTracking", dbOpenSnapshot)
    Do While Not rs.EOF
        If Not IsNull(rs!JobOrderID) And Not IsNull(rs!ShippedDate) Then shipmentKeys(CStr(rs!JobOrderID) & "|" & Format(CDate(rs!ShippedDate), "yyyymmddhhnnss")) = True
        rs.MoveNext
    Loop
    rs.Close
End Sub

Private Function ShippingJobID(ByVal jobByConsult As Object, ByVal jobByPO As Object, ByVal consultValue As Variant, ByVal poValue As Variant) As Long
    Dim key As String
    If Not IsNull(consultValue) Then
        If Len(Trim$(CStr(consultValue))) > 0 Then
            If IsNumeric(consultValue) Then
                key = "C:" & CStr(CLng(Val(consultValue)))
                If jobByConsult.Exists(key) Then ShippingJobID = CLng(jobByConsult(key))
            End If
            Exit Function
        End If
    End If
    If Not IsNull(poValue) Then
        key = "P:" & UCase$(Trim$(CStr(poValue)))
        If jobByPO.Exists(key) Then ShippingJobID = CLng(jobByPO(key))
    End If
End Function
...
Private Function SqlValue(ByVal value As Variant) As String
    If IsNull(value) Then SqlValue = "Null" Else SqlValue = "'" & SqlText(CStr(value)) & "'"
End Function

Private Function ValueFromStage(ByVal rs As DAO.Recordset, ByVal sourceRef As String) As Variant
    Dim fieldName As String
    If sourceRef = "Null" Then ValueFromStage = Null: Exit Function
    fieldName = Replace(Replace(sourceRef, "S.[", ""), "]", "")
    ValueFromStage = rs.Fields(fieldName).Value
End Function

Private Function FindShippingJob(ByVal db As DAO.Database, ByVal consultValue As Variant, ByVal poValue As Variant) As DAO.Recordset
    Dim sql As String
    If Not IsNull(consultValue) Then
        If IsNumeric(consultValue) Then
            If CLng(Val(consultValue)) <> 0 Then
                sql = "SELECT * FROM JobOrder WHERE ConsultNumber=" & CStr(CLng(Val(consultValue)))
            End If
        End If
    End If
    If Len(sql) = 0 Then
        If Not IsNull(poValue) Then
            If Len(Trim$(CStr(poValue))) > 0 Then
                sql = "SELECT * FROM JobOrder WHERE PONumber='" & SqlText(CStr(poValue)) & "'"
            End If
        End If
    End If
    If Len(sql) = 0 Then Set FindShippingJob = Nothing: Exit Function
    Set FindShippingJob = db.OpenRecordset(sql, dbOpenDynaset)
End Function

Private Sub InsertShipmentsByMatch(ByVal db As DAO.Database, ByVal joinCondition As String, ByVal sourceWhere As String, ByVal shipped As String, ByVal tracking As String, ByVal invoice As String, ByVal frame As String, ByVal lens As String)
    db.Execute "INSERT INTO ShipmentTracking (JobOrderID,ShippedDate,TrackingNumber,InvoiceNumber,FrameName,LensDescription) " & _
        "SELECT J.JobOrderID," & shipped & "," & tracking & "," & invoice & "," & frame & "," & lens & _
        " FROM [" & STAGE_SHIPPING & "] AS S INNER JOIN JobOrder AS J ON " & joinCondition & _
        " WHERE " & sourceWhere & " AND " & shipped & " Is Not Null " & _
        "AND NOT EXISTS (SELECT * FROM ShipmentTracking AS T WHERE T.JobOrderID=J.JobOrderID AND T.ShippedDate=" & shipped & ")", dbFailOnError
End Sub

Private Sub CloseJobsByMatch(ByVal db As DAO.Database, ByVal joinCondition As String, ByVal sourceWhere As String, ByVal shipped As String, ByVal frame As String, ByVal lens As String)
    db.Execute "UPDATE JobOrder AS J INNER JOIN [" & STAGE_SHIPPING & "] AS S ON " & joinCondition & _
        " SET J.ClosedDate=" & shipped & ", J.JobStatus='Closed', " & _
        "J.FrameName=Nz(" & frame & ",J.FrameName), J.LensDescription=Nz(" & lens & ",J.LensDescription) " & _
        "WHERE " & sourceWhere & " AND " & shipped & " Is Not Null", dbFailOnError
End Sub
...
Private Function HasField(ByVal db As DAO.Database, ByVal tableName As String, ByVal fieldName As String) As Boolean
    On Error Resume Next
    HasField = (Len(db.TableDefs(tableName).Fields(fieldName).Name) > 0)
    Err.Clear
    On Error GoTo 0
End Function
